...
def display_dietary_plan(plan_content):
    with st.expander("📋 Ihr individueller Ernährungsplan", expanded=True):
        col1, col2 = st.columns([2, 1])
        
        with col1:
            st.markdown("### 🎯 Warum dieser Plan funktioniert")
            st.info(plan_content.get("why_this_plan_works", "Informationen nicht verfügbar"))
            st.markdown("### 🍽️ Essensplan")
            st.write(plan_content.get("meal_plan", "Plan nicht verfügbar"))
        
        with col2:
            st.markdown("### ⚠️ Wichtige Überlegungen")
...
def display_fitness_plan(plan_content):
    with st.expander("💪 Ihr individueller Fitnessplan", expanded=True):
        col1, col2 = st.columns([2, 1])
        ...
        with col1:
            ...
            st.write(plan_content.get("routine", "Routine nicht verfügbar"))
...
def main():
    ...
    st.markdown("""
        ...
        Sie erhalten personalisierte Ernährungs- und Fitnesspläne, die auf Ihre Ziele und Vorlieben zugeschnitten sind.
        Unser KI-gestütztes System berücksichtigt Ihr individuelles Profil, um den perfekten Plan für Sie zu erstellen.
        </div>
    """, unsafe_allow_html=True)
    ...
    with st.sidebar:
        ...
        gemini_api_key = st.text_input(
            "Gemini API Key",
            type="password",
            help="Geben Sie Ihren Gemini-API-Schlüssel ein, um auf den Dienst zuzugreifen"
        )
        
        if not gemini_api_key:
            st.warning("⚠️ Bitte geben Sie Ihren Gemini API Key ein, um fortzufahren")
            st.markdown("[Holen Sie sich Ihren API-Schlüssel hier](https://aistudio.google.com/apikey)")
            return
    ...
    if gemini_api_key:
        ...
        with col1:
            ...
            dietary_preferences = st.selectbox(
                ...
                help="Wählen Sie Ihre Ernährungspräferenz"
            )
